...
// Discovery agent state -- tracks analysis loop per recording
const discoveryState = {
  lastAnalysisTime: null,
  analysisIntervalMs: 3 * 60 * 1000, // 3 minutes
...
async function maybeRunDiscoveryAnalysis(noteId) {
  const now = Date.now();

  // Guard: don't overlap analysis calls
  if (discoveryState.isAnalyzing) return;

  // Guard: respect interval
  if (discoveryState.lastAnalysisTime &&
      (now - discoveryState.lastAnalysisTime) < discoveryState.analysisIntervalMs) return;

  // Guard: need at least 60 seconds of recording
  if (discoveryState.recordingStartTime &&
      (now - discoveryState.recordingStartTime) < 60000) return;

  discoveryState.isAnalyzing = true;
  discoveryState.lastAnalysisTime = now;
  ...
  try {
    ...
    if (!meeting || !meeting.transcript || meeting.transcript.length < 3) {
      discoveryState.isAnalyzing = false;
      return;
    }

    const elapsedMinutes = discoveryState.recordingStartTime
      ? Math.round((now - discoveryState.recordingStartTime) / 60000)
      : 0;

    console.log(`[Discovery] Running analysis at ${elapsedMinutes}m, ${meeting.transcript.length} transcript entries`);

    const analysis = await analyzeDiscoveryCoverage(meeting.transcript, discoveryState.previousAnalysis);
    console.log(`[Discovery] Coverage: ${analysis.overall_score}/24, Brief readiness: ${analysis.brief_readiness_pct}%`);

    // Send IPC update to renderer
    if (mainWindow && !mainWindow.isDestroyed()) {
      mainWindow.webContents.send('discovery-update', { meetingId: noteId, analysis });
    }

    // Real-time nudges go to the scoreboard UI (via IPC above).
    // Slack is reserved for the final post-call report only.

    discoveryState.previousAnalysis = analysis;
  } catch (err) {
    console.error('[Discovery] Analysis error:', err.message);
  } finally {
    discoveryState.isAnalyzing = false;
  }
}
...
async function initSDK() {
  ...
  // Request screen capture and system audio permissions
  // This triggers the macOS permission dialogs
  try {
    console.log('[Permissions] Requesting screen_capture...');
    await RecallAiSdk.requestPermission('screen_capture');
    console.log('[Permissions] screen_capture granted');
  } catch (e) {
    console.error('[Permissions] screen_capture error:', e.message);
  }
  try {
    console.log('[Permissions] Requesting system_audio...');
    await RecallAiSdk.requestPermission('system_audio');
    console.log('[Permissions] system_audio granted');
  } catch (e) {
    console.error('[Permissions] system_audio error:', e.message);
  }
  ...
  RecallAiSdk.addEventListener('recording-ended', async (evt) => {
    ...
    try {
      ...
      // Run final discovery analysis
      try {
        const windowId = evt.window.id;
        const activeMeeting = global.activeMeetingIds && global.activeMeetingIds[windowId];
        if (activeMeeting && activeMeeting.noteId) {
          const meetingsData = await fileOperationManager.readMeetingsData();
          const meeting = meetingsData.pastMeetings.find(m => m.id === activeMeeting.noteId);
          if (meeting && meeting.transcript && meeting.transcript.length > 0) {
            const durationMinutes = discoveryState.recordingStartTime
              ? Math.round((Date.now() - discoveryState.recordingStartTime) / 60000)
              : 0;
            console.log(`[Discovery] Running final analysis (${durationMinutes}m call, ${meeting.transcript.length} entries)`);
            const finalAnalysis = await analyzeDiscoveryCoverage(meeting.transcript, null);

            // Store analysis in meeting object
            meeting.discoveryAnalysis = finalAnalysis;
            await fileOperationManager.scheduleOperation(async (data) => {
              const m = data.pastMeetings.find(x => x.id === activeMeeting.noteId);
              if (m) m.discoveryAnalysis = finalAnalysis;
              return data;
            });
        ...
        discoveryState.previousAnalysis = null;
        discoveryState.lastAnalysisTime = null;
        discoveryState.nudgeHistory = [];
        discoveryState.recordingStartTime = null;
      } catch (discoveryErr) {
        console.error('[Discovery] Final analysis error:', discoveryErr.message);
      }
  ...
  RecallAiSdk.addEventListener('realtime-event', async (evt) => {
    // Log all events except raw audio data dumps
    if (evt.event === 'audio_mixed_raw.data') {
      console.log("[Audio] Raw audio chunk received");
...
// Handle manual discovery analysis trigger
ipcMain.handle('runDiscoveryAnalysis', async (event, meetingId) => {
  try {
    console.log(`[Discovery] Manual analysis requested for meeting: ${meetingId}`);
    const meetingsData = await fileOperationManager.readMeetingsData();
    ...
    console.error('[Discovery] Manual analysis error:', error);
    return { success: false, error: error.message };
  }
});
...
ipcMain.handle('startManualRecording', async (event, meetingId) => {
  try {
    ...
    try {
      ...
      // Start discovery analysis timer
      discoveryState.recordingStartTime = Date.now();
      discoveryState.previousAnalysis = null;
      discoveryState.nudgeHistory = [];
      console.log('[Discovery] Recording started, analysis timer armed');
...
async function createMeetingNoteAndRecord(platformName) {
  ...
  try {
    ...
    try {
      ...
      console.log('[Recording] startRecording succeeded. Waiting for realtime events...');
...
async function processTranscriptData(evt) {
  try {
    ...
    // Trigger discovery analysis if interval has elapsed
    maybeRunDiscoveryAnalysis(noteId);
  } catch (error) {
    console.error('Error processing transcript data:', error);
  }
}

// Function to generate AI summary from transcript via claude CLI
async function generateMeetingSummary(meeting, progressCallback = null) {
  try {
    ...
    const prompt = `You are an AI assistant that summarizes meeting transcripts. Format your response using this structure:
...
# Action Items
- [Action item] - [Responsible person if mentioned]
...
${transcriptText}`;
    ...
    // progressCallback not supported with CLI -- just return final result
    const summary = await callClaude(prompt, 'sonnet');
    console.log('AI summary generated successfully via claude CLI');
    return summary;
  } catch (error) {
    console.error('Error generating meeting summary:', error);
