...
document.addEventListener('DOMContentLoaded', async () => {
  ...
  const discoveryReadiness = document.getElementById('discoveryReadiness');
  const discoveryNudge = document.getElementById('discoveryNudge');
  const nudgeQuestion = document.getElementById('nudgeQuestion');
  const discoveryPyramid = document.getElementById('discoveryPyramid');
  const discoveryBlindSpots = document.getElementById('discoveryBlindSpots');
  const discoveryDot = document.querySelector('.discovery-dot');

  // Show scoreboard when recording starts
  window.electronAPI.onRecordingStateChange((data) => {
    if (data.state === 'recording') {
      discoveryPanel.classList.remove('hidden');
    } else if (data.state === 'idle' || data.state === 'stopping') {
      // Keep panel visible briefly after stop so they can see final state
      setTimeout(() => {
  ...
  // Update scoreboard when discovery analysis completes
  window.electronAPI.onDiscoveryUpdate((data) => {
    const { analysis } = data;
    if (!analysis || !analysis.coverage) return;

    // Show the panel
    discoveryPanel.classList.remove('hidden');

    // Update readiness percentage
    const pct = analysis.brief_readiness_pct || 0;
    discoveryReadiness.textContent = `${pct}%`;
    discoveryReadiness.style.color = pct >= 60 ? '#4ade80' : pct >= 30 ? '#facc15' : '#ef4444';

    // Update category bars
    Object.entries(analysis.coverage).forEach(([key, val]) => {
      const catEl = document.querySelector(`.disc-cat[data-cat="${key}"]`);
      if (!catEl) return;

      const fill = catEl.querySelector('.disc-cat-fill');
      const label = catEl.querySelector('.disc-cat-label');

      // Remove old score classes, add new one
      fill.className = 'disc-cat-fill score-' + (val.score || 0);

      // Color the label based on score
      label.className = 'disc-cat-label';
      if (val.score <= 1) label.classList.add('weak');
      else if (val.score >= 3) label.classList.add('strong');
    });

    // Update nudge
    if (analysis.nudge && analysis.nudge.question) {
      discoveryNudge.classList.remove('hidden');
      nudgeQuestion.textContent = analysis.nudge.question;
    }

    // Update meta
    if (analysis.pyramid_layer) {
      discoveryPyramid.textContent = `Pyramid: Layer ${analysis.pyramid_layer}`;
    }
    const spots = analysis.blind_spots_detected || [];
    if (spots.length > 0) {
      discoveryBlindSpots.textContent = spots.slice(0, 2).join(', ');
    }

    // Flash the dot to show analysis just ran
    if (discoveryDot) {
      discoveryDot.classList.add('analyzing');
      setTimeout(() => discoveryDot.classList.remove('analyzing'), 2000);
    }
  });
